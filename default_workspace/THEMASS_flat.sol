...
// File: THEMASSDividendTracker.sol
...
contract THEMASSDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "THEMASS_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "THEMASS_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main THEMASS contract.");
    }
...
contract THEMASS is ERC20, Ownable {
    
    address WHALEPOOL = 0xe48199D92c35e4BF7832D932d2797c0ec3C4390B;
...
//Max sell amount
    uint256 public maxSellTransactionAmount = 10**13 * (10**6);
    uint256 public swapTokensAtAmount = 3 * 10**15 * (10**6);
    
    
    uint256 public immutable BNBRewardsFee;
    uint256 public immutable liquidityFee;
    uint256 public immutable totalFees;
    uint256 public immutable WHALEPOOLRewardsFee;
    ...
    constructor() ERC20("Themass", "TMS") {
        uint256 _liquidityFee = 4;
        uint256 _BNBRewardsFee = 10;
        uint256 _WHALEPOOLRewardsFee = 6;

        BNBRewardsFee = _BNBRewardsFee;
        liquidityFee = _liquidityFee;
        WHALEPOOLRewardsFee = _WHALEPOOLRewardsFee;
        totalFees = _BNBRewardsFee.add(_liquidityFee);

    	dividendTracker = new THEMASSDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "THEMASS: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "THEMASS: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "THEMASS: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "THEMASS: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "THEMASS: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "THEMASS: Cannot update gasForProcessing to same value");
